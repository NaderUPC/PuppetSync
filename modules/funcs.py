...
def sync_os(puppet_ep: puppet.Puppet, cmdb_ep: cmdb.CMDB, hostname: str, cmdb_os: str = None) -> None:
    """
    ...
    if puppet_ep.os(hostname) != cmdb_os or not cmdb_os:
        cmdb_ep.link_host_sw(hostname, puppet_ep.os(hostname))
    else:
        print("Already in sync!\n")

def sync_sw(puppet_ep: puppet.Puppet, cmdb_ep: cmdb.CMDB, hostname: str, cmdb_sw: str = None) -> None:
    """
    (...)
    """
